...
describe('UI 基础组件无障碍状态', () => {
  it('Button disabled 与 loading 态不可触发，也不可聚焦', () => {
    const onClick = vi.fn();
    const { rerender } = render(
      <Button controlId="start-session" disabled disabledReason="设备检查尚未完成" onClick={onClick}>
        开始面试
      </Button>,
    );
    const button = screen.getByRole('button', { name: '开始面试' });
    fireEvent.click(button);
    expect(onClick).not.toHaveBeenCalled();
    expect(button.getAttribute('tabindex')).toBe('-1');
    expect(screen.getByText('设备检查尚未完成')).toBeTruthy();

    rerender(
      <Button controlId="start-session" loading busyLabel="正在准备" onClick={onClick}>
        开始面试
      </Button>,
    );
    fireEvent.click(screen.getByRole('button', { name: /开始面试/ }));
    expect(onClick).not.toHaveBeenCalled();
    expect(screen.getByRole('button', { name: /开始面试/ }).getAttribute('aria-busy')).toBe('true');
  });
