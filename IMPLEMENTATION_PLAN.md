...
> **任务状态（2026-08-03 更新）**：TASK-085 已实现（FR-039，US-08 场景 4）——
> `services/adminapi` 客服工单：默认最小可见（账户状态/订单/额度/故障代码/
> 用户提交材料，工单不含逐字稿与媒体正文）；逐字稿需用户针对会话授权
> （会话范围+有效期，到期自动失效，访问/阻断均写审计）；媒体访问需用户申请 +
> 双人审批（同一审批人去重、用户本人不可自批、可拒绝）；工单解决/关闭。
> 迁移 `0073_support_tickets.sql`；openapi 增加 tickets/transcript-auth/
> media-requests/approve/reject 端点与 schema。
> 服务正常、异常、最小可见、授权到期、双人审批、角色门禁测试齐备（新增 4 用例），
> gofmt/vet 通过。
...
|---|---|---|---|---|---|
| OD-01 | WebRTC/数字人/ASR/TTS/LLM 主备供应商 | 技术负责人、AI 负责人、安全负责人 | 中英文质量、实时 SLO、数据区、授权、成本、故障切换对比 | Phase 0 退出前 | **已决策 2026-08-04**（项目负责人授权代签；签字材料见 docs/decisions/OD-01-signoff.md，ADR-0006；弱网与 TTS 备选实测定稿挂起） |
