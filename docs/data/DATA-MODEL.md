...
- 33 张核心业务表（含 TASK-012 的上传/扫描表与 TASK-013 的 `resume_parse_attempts`）、追加式账本约束、索引与分区策略。
...
3. 与 `docs/domain/DOMAIN-MODEL.md` 实体覆盖核对（33 表 ↔ 全部实体）；与 `ai/schemas/` 内容字段命名抽样比对。
