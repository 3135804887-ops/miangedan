...
- TTS：生产候选火山豆包 API / CosyVoice 2；edge-tts 仅本地开发/演示（cn 区生产红线否决）
...
- TTS（edge_tts）：**红线否决**（cn 区生产禁止：非官方接口 + 数据出境 + 无 DPA）；仅作开发/演示
...
2. TTS 生产选型：火山豆包 API 或 CosyVoice 2 待注册/部署后实测定稿；edge-tts 不得进入 cn 区生产。
...
签字完成后动作：① 更新 `IMPLEMENTATION_PLAN.md` OD-01 状态为已决策；② 新增 ADR（自建媒体/AI 链路与供应商适配）；③ `docs/testing/RELEASE-CHECKLIST.md` 勾选 OD-01 归档项；④ 解锁窗口 2（TASK-091/092/096）。
