...
|---|---|---|---|---|
| 账户与身份 | 邮箱/第三方 subject 的区域密钥摘要、年龄状态、注册告知版本证据 | restricted | 至账户删除 | 否 |
...
| 邮件/通知 | 投递时瞬时邮箱地址、验证码与有效期模板变量（无用户内容/业务令牌/媒体） | 仅供应商最小投递日志；身份库不保存邮箱/验证码明文 | 区域化 | DPA |
| 错误追踪 | 匿名技术指标、堆栈（脱敏） | 否 | 区域化端点 | 禁止正文上报的 SDK 级过滤 |
| 对象存储 | 原始文件、导出物、授权媒体 | 是（加密） | 区域内桶 | 静态加密 + 区域锁定 |

TASK-010 身份链路边界：邮箱地址仅在调用所属数据区 `services/notify` 时瞬时使用；数据库只保存
区域密钥 HMAC 摘要。OAuth 授权码、验证码、短期验证证明、业务令牌和刷新令牌不得以明文持久化、
写日志或进入错误响应；公开账户响应只包含提供商类型与验证时间，不返回邮箱或第三方 subject。

TASK-011 授权链路边界：`services/consent` 只接收 TASK-010 业务令牌并核对部署/令牌 `data_region`；
scope 为 assignment_id、数据类别、媒体类别、通知渠道四类封闭元数据，证据仅含文案/隐私政策版本、
展示时间和枚举化 UI 上下文及 SHA-256 哈希，不接收或记录正文、令牌、媒体、邮箱等敏感值。每次授予/
撤回均追加新 ConsentGrant 版本并与 content-free AccessAudit 同事务提交；审计失败整体回滚。指标、
跨度事件与结构化日志端口只输出固定低基数字段，不含用户标识、scope/hash、幂等键或授权证据。
...
技术执行：无当前记录、已撤回、已到期或授权服务状态不确定均按未授权处理；原始音视频每次在线判定
均通过最小权限身份适配器重新核对成人资格，授权最长 30 天；机构共享必须限定 assignment_id、
数据类别和未来到期时间。撤回成功返回后在线判定
立即为拒绝，历史证据行不修改；模型训练撤回不改变核心服务授权。
...
| 授权状态不确定（同步延迟/服务故障） | 在线访问按"未授权"保守处理；授权写入失败整体回滚，恢复后以同一幂等键重试并对账 |
