...
| TM-02 | 业务 API | Tampering | **恶意文件上传**：病毒、宏、压缩炸弹、扩展名伪装、解析器漏洞利用 | 服务端失陷、数据泄露 | 无出站网络、一次性、只读根文件系统、无凭证挂载的隔离沙箱；类型嗅探（不信任扩展名）、大小/解压比限制、杀病毒与宏检测、解析器加固更新；证明不完整即 fail-closed | 0day 解析器漏洞 | 病毒/宏/压缩炸弹/伪装/超限/损坏/加密合成矩阵（全部拒绝且给具体原因）；沙箱逃逸测试 |
