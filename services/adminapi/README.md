...
- **客服工单**：默认最小可见（账户状态/订单/额度/故障代码/用户提交材料，
  不含逐字稿与媒体正文）；逐字稿需用户针对会话授权（范围+有效期，到期失效）；
  媒体访问需用户申请+双人审批（去重、不可自批、可拒绝）；访问/阻断写审计。
- 迁移 `0073_support_tickets.sql`。

## 规划（后续任务）

- EPIC-10 上线验证（Phase 0–4 门槛）任务在 IMPLEMENTATION_PLAN 另行推进。
