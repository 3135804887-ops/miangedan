...
- **轮次结果流**（TASK-054）：SCR-10 / FR-021、FR-022
  - `BuildRoundResultView`：通过态固定祝贺文案"恭喜你通过本轮面试，已进入下一轮"、
    总分/60 分线/关键维度通过情况/优势与注意点/进度/下一轮预告（角色/重点/难度/
    时长，不含题目）；失败态阻断下一轮、生成从第 1 轮到当前轮累计纪要并提供
    复盘练习/正式重试/报告/复核入口；评估未完成明确"这不是失败"，按原因
    （证据不足/系统故障/用户结束）展示说明，系统责任展示额度已返还；
  - 红线：任何结果视图不携带后续轮次完整答案或考点内容。
