...
- **Pro 订阅生命周期**（TASK-064，FR-033，US-06 场景 5）：自动续费单独勾选并
  记录同意条款（变化须重新同意）；扣款前提醒 + 7 天窗口扣款；结转 ≤1 账期、
  总余额 ≤2×月额度；取消续费权益保留至账期结束；到期 SUB_EXPIRED 历史保留，
  进行中的正式轮次可正常结束。迁移 `0063_pro_renewals.sql`。
- **发票与税费**（TASK-065，FR-033，US-06 场景 11）：区域定价配置（cn CNY/6%
  增值税/合规发票；eu EUR/19% VAT/收据；intl USD/0% 明示税费/收据）；开票幂等
  （同一订单一份、价税一致）；中国区发票可作废（合规），国际区税费明示收据。
  迁移 `0064_invoices.sql`。

## 规划（后续任务）

- EPIC-08 机构（TASK-070~074）与 EPIC-09 治理（TASK-080~085）在 org/adminapi 推进。
